...
//       <button
//         onClick={() => setMenuOpen(!menuOpen)}
//         className="flex flex-col p-1 justify-between w-6 h-6 absolute right-6 top-1 md:hidden focus:outline-none z-50"
//         aria-label="Toggle menu"
//         aria-controls="mobile-menu"
//         aria-expanded={menuOpen}
//       >
...
//         {/* Title (left corner) */}
//         <h1
//           className={`md:text-2xl  pr-9  text-lg  font-bold absolute left-16 top-3 transform -translate-y-1/2 ${
//             darkMode ? "text-white" : "text-gray-900"
//           }`}
//         >
//           {title}
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        ...
        {/* Desktop nav */}
        <nav className="hidden pl-10 md:flex items-center justify-center md:space-x-10 space-x-3 font-medium">
          {navItems.map(({ to, label }) => (
            <NavLink
              key={to}
              to={to}
      ...
      <div
        id="mobile-menu"
        className={`fixed top-0 right-0 h-full w-56 backdrop-blur-md shadow-lg transform transition-transform duration-300 ease-in-out z-50 ${
          menuOpen
            ? `${
                darkMode
      ...
      >
        <nav className="flex flex-col p-6 space-y-6 font-medium">
          ...
          <button
            ...
            aria-label="Toggle dark mode"
            aria-pressed={darkMode}
            className={`relative inline-flex items-center h-8 rounded-full w-14 transition-colors duration-300 mt-4 self-start ${
              darkMode ? "bg-indigo-600" : "bg-gray-300"
